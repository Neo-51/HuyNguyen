 ...
 * Breadcrumbs
 */
if ( ! function_exists( 'mikael_get_breadcrumbs' ) ) {
	function mikael_get_breadcrumbs() {
		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'mikael' ); // text for the 'Home' link
		$text['category'] = esc_html__( 'Archive by Category "%s"', 'mikael' ); // text for a category page
		$text['search']   = esc_html__( 'Search Results for "%s"', 'mikael' ); // text for a search results page
		$text['tag']      = esc_html__( 'Posts Tagged "%s"', 'mikael' ); // text for a tag page
		$text['author']   = esc_html__( 'Articles Posted by %s', 'mikael' ); // text for an author page
		$text['404']      = esc_html__( 'Error 404', 'mikael' ); // text for the 404 page

		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'mikael' ); // text for the 'Home' link
		if( get_option( 'page_for_posts' ) !=='0' ){
		$text['category'] = '<a href="'.get_permalink( get_option( 'page_for_posts' ) ) . '">' . esc_html__( 'Blog', 'mikael' ) . '</a> <span class="sep">-</span> ' . esc_html__( '%s', 'mikael' ); // text for a category page
		}
		$text['search']   = esc_html__( 'Search Results for "%s"', 'mikael' ); // text for a search results page
		$text['tag']      = esc_html__( '%s', 'mikael' ); // text for a tag page
		$text['author']   = esc_html__( 'Articles Posted by %s', 'mikael' ); // text for an author page
		$text['404']      = esc_html__( '404', 'mikael' ); // text for the 404 page
		$text['page']     = esc_html__( 'Page %s', 'mikael' ); // text 'Page N'
		$text['cpage']    = esc_html__( 'Comment Page %s', 'mikael' ); // text 'Comment Page N'
		$wrap_before      = '<nav class="vlt-breadcrumbs vlt-has-link-effect">'; // the opening wrapper tag
		...
		if ( is_home() || is_front_page() ) {
			...
			if ( is_category() ) {
				...
				if ( get_post_type() != 'post' ) {
					...
					if ( get_option( 'page_for_posts' ) !=='0' ) {
						$output .= '<a href="'.get_permalink( get_option( 'page_for_posts' ) ) . '">' . esc_html__( 'Blog', 'mikael' ) . '</a> <span class="sep">-</span> '. $cats;
					} else {
						$output .= $cats;
					}
